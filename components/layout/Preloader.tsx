...
function isoPoints(cx: number, cy: number, s: number) {
    ...
    return {
        ...
        left: [[cx - s, cy + h], [cx, cy + h * 2], [cx, cy + h * 2 + s], [cx - s, cy + h + s]],
        right: [[cx + s, cy + h], [cx, cy + h * 2], [cx, cy + h * 2 + s], [cx + s, cy + h + s]],
        shadowY: cy + h * 2 + s,
    };
}

function drawPoly(ctx: CanvasRenderingContext2D, pts: number[][], color: string) {
    ctx.beginPath();
